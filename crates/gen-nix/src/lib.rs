...
mod tests {
    ...
    #[test]
    fn renders_a_workspace_in_crate2nix_schema() {
        use gen_types::{
        ...
        // Lambda header is the crate2nix signature
        assert!(full.contains("{ nixpkgs ? <nixpkgs>"));
        assert!(full.contains("buildRustCrateForPkgs ? pkgs: pkgs.buildRustCrate"));
        // Body is `rec { rootCrate = ...; workspaceMembers = ...; ... }`
        assert!(full.contains("rec "));
        assert!(full.contains("rootCrate"));
        assert!(full.contains("workspaceMembers"));
        assert!(full.contains("allWorkspaceMembers"));
        // internal block with crates table
        assert!(full.contains("internal = rec"));
        assert!(full.contains("crates"));
        // Resolved entry under package-name key (no -version suffix)
        assert!(full.contains("\"serde\" = rec"));
        // Workspace member also under its name
        assert!(full.contains("\"demo\" = rec"));
        // sha256 stripped of `sha256:` prefix
        assert!(full.contains("sha256 = \"abc\""));
        // Authors emitted
        assert!(full.contains("acme <a@x>"));
        // Vendored helpers embedded
        assert!(full.contains("buildRustCrateWithFeatures"));
        assert!(full.contains("expandFeatures"));
        // File is well-formed: starts with comment, ends with `}`
        assert!(full.starts_with("# Generated by gen-nix"));
